...
# ? Instructor + OpenAI package: wrap openAI client within instructor.patch
# this ensures that the LLM will give us structured output
...
async def async_generate_visualization_plan(table_info, question):
    ...
    plan = await async_client.chat.completions.create(
        ...
        stream=True,
        # ? instructor package ensures structured output from LLM
...
class VisualizationTask(BaseModel):
    ...
    def run(self):
        ...
        if self.type == VisualizationType.BAR_CHART:
            fig = get_bar_chart(
                data=data, title=self.title, barmode=self.parameters.get(
                    "bar_mode")
            )
            return fig
...
input_field = st.text_input("Enter a question", value=st.session_state.get(
    "user_input"), key="input_field_value", on_change=set_user_input_from_field)

example_questions = ["Price of BTC in the last 30D", "Daily volumes for ETH and BTC in March 2024",
                     "Forecast SOL prices for next 7 days, based on last 14 days"]
